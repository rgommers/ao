...
- **Severity:** HIGH
- **Status:** fixed — added `std::mutex` to `UKernelConfigRegistrationTable`; `register_ukernel_config` and `get_ukernel_config` now take a `std::lock_guard`. Follow-up commit also made `register_ukernel_config` idempotent on duplicate keys, fixing a TOCTOU race at the call site in `select_ukernel_config` (get-miss-register without a spanning lock).
- **MT test coverage:** yes — `test/free_threading/test_cpu_kernel_selector_concurrent.py::test_linear_8bit_act_4bit_weight_concurrent[4|8]` runs on both x86 (fallback kernels: registration table is populated, kernel stubs throw a known RuntimeError that the test catches) and aarch64 (real kernels). The test caught the call-site TOCTOU described above before the idempotent-register fix landed, exposing a real bug the static audit missed.
...
- **Category:** native-cache
- **Severity:** HIGH
- **Status:** fixed — same `std::mutex` treatment as F-01 plus the same idempotent-register follow-up; doc comment updated from "thread-unsafe" to "thread-safe"
- **MT test coverage:** scaffold placeholder — `test/free_threading/test_cpu_kernel_selector_concurrent.py::test_groupwise_lowbit_weight_lut_concurrent` carries the structure (skip-on-x86 marker, N-thread barrier, body) but is `@pytest.mark.skip`'d pending verified inputs for `_pack_groupwise_4bit_weight_with_lut`. A working input set can be derived from a single forward of `quantize_(model, GroupwiseLutWeightConfig(...))` (see `test/prototype/test_groupwise_lowbit_weight_lut_quantizer.py`). The mutex + idempotent-register pattern is identical to F-01 and the F-01 test has validated the underlying behavior — coverage here is reduced because the op has no x86 fallback (`select_ukernel_config` throws unconditionally on non-aarch64).
